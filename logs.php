...
if ($_SESSION['roleuser'] == 'operador' || $_SESSION['roleuser'] == 'admin'){
  ...
  echo"<div class='row'>
      <div class='col-md-4'>
        <div class='form-group' id='form-logDate'>
          <label class='control-label' for='logDate'>Logs da data:</label>
          <input class='form-control' id='logDate' value='" .  $dateTxtInput . "'>
        </div>
        <input type='button' value='Carregar' onclick='changeDate();' class='btn btn-primary'>
      </div>
    </div>
  <hr>";
...
function changeDate(){
  ...
  $.post("pastLogs.php", { logDate: logDate },
  function(data) {
    $('#logArea').html(data);
  });
}
